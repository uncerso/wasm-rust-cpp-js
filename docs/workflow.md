...
- **Shared-shape change → enumerate ALL consumers in Self-Review** — a plan that changes a shared type/schema/metric (schema-version bump, swapping an acceptance metric) MUST grep the whole repo for every consumer in its Self-Review — test fixtures, dependent UI cues, downstream assemblers — not just the files it names. A v1→v2 schema bump this phase missed two v1 fixtures (`render`/`aggregate` tests) and dropped the amber row's cell-highlight cue (moved cv→relSem, but the highlight lived only in the renderer), both caught late as fix-on-fix (`336cb48`, `b5e452e`).
