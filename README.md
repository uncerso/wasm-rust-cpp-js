...
Size читает `composition` из `dist/*/meta.json` (rust/raw покрыты; cpp/wasi-sdk, bindgen, emscripten, js деградируют до одного бара с пометкой — расширение атрибуции на эти тулчейны отложено, см. roadmap `size-attr-toolchain-coverage`). Каждый JSON-результат прогоняется через `BenchResultSchema.parse` перед агрегацией — невалидный файл будет ошибкой.
