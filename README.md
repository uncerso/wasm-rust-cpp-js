...
- **`navigator.platform`/таймеры в Firefox.** Firefox 110+ выдаёт пустой `navigator.platform` и квантует `performance.now()` до миллисекунды для anti-fingerprinting. На S-размере это означает, что warm timings часто `0.000` или `1.000` — реально pre-rounding значения недоступны. На M-размере влияние есть, но обычно укладывается в < 1% (тайминг 50–130 мс). Для строгих сравнений между движками лучше брать M или L.
