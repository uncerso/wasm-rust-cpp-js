...
export async function detectActual(): Promise<Record<string, string>> {
    ...
    try {
        out.rustc = (await capture("rustc", ["--version"])).trim();
    } catch {}
    try {
        out["wasm-pack"] = (await capture("wasm-pack", ["--version"])).trim();
    } catch {}
    try {
        out["wasm-opt"] = (await capture("wasm-opt", ["--version"])).trim();
    } catch {}
    try {
        const emccOut = await capture("emcc", ["--version"]);
        const firstLine = emccOut.split("\n")[0];
        if (firstLine) {
            out.emcc = firstLine.trim();
        }
    } catch {}
